...
describe("cognee-openclaw agent_end — workspace resolution", () => {
  ...
  it("uses ctx.workspaceDir when provided", async () => {
    const { eventHandlers } = createApi();
    await eventHandlers["agent_end"]({ success: true }, { agentId: "default", workspaceDir: "/custom-workspace" });
    expect(mockCollectMemoryFiles).toHaveBeenCalledWith("/custom-workspace");
  });

  it("falls back to resolvedWorkspaceDir (set during startup) when ctx.workspaceDir is absent", async () => {
    const { eventHandlers } = createApi();
  ...
  it("prefers ctx.workspaceDir over startup resolvedWorkspaceDir", async () => {
    const { eventHandlers } = createApi();
    await eventHandlers["agent_end"]({ success: true }, { agentId: "default", workspaceDir: "/override" });
    expect(mockCollectMemoryFiles).toHaveBeenCalledWith("/override");
    expect(mockCollectMemoryFiles).not.toHaveBeenCalledWith("/startup-workspace");
  });
});
...
describe("cognee-openclaw agent_end — multi-scope sync", () => {
  ...
  it("passes the runtimeAgentId through to syncFilesScoped", async () => {
    mockLoadScopedSyncIndexes.mockResolvedValue({} as ScopedSyncIndexes);
    mockCollectMemoryFiles.mockResolvedValue([makeFile("memory/tools.md", "h1")]);
    mockSyncFilesScoped.mockResolvedValue({ added: 1, updated: 0, skipped: 0, errors: 0, deleted: 0 });

    const { eventHandlers } = createApi({ companyDataset: "acme-shared", agentId: "coder" });
    await eventHandlers["agent_end"]({ success: true }, { agentId: "reviewer", workspaceDir: "/ws" });

    const lastArg = (mockSyncFilesScoped as jest.Mock).mock.calls[0].at(-1);
    expect(lastArg).toBe("reviewer");
  });
});
